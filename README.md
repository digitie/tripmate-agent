...
`tripmate-agent`는 유튜브의 방대한 여행 콘텐츠 영상들을 지능적으로 탐색하고, Gemini API를 통해 영상 내용 요약 및 추천 여행지를 자동으로 발라내어 데이터베이스화하는 웹 애플리케이션입니다. 

수집된 장소 정보는 외부 REST API(Geocoding)를 통해 정밀 주소와 위경도로 보정된 후, 프론트엔드의 `maplibre-vworld-js` 지도 상에 실시간으로 매핑됩니다. 사용자는 리스트 및 지도 뷰를 통해 가독성 높은 정보를 확인하며, 특정 여행지를 심층 조사하는 Gemini Deep Research 모듈을 트리거하여 지속해서 고도화된 여행 지식을 구축할 수 있습니다.
...
- **위치 보정 및 정보 보완 (ETL 3단계)**: 텍스트 형태로 적재된 불완전한 주소명을 카카오 로컬/네이버 맵 API 등 외부 REST API와 연동해 실제 경위도 좌표 및 표준 주소로 치환 및 가공합니다.
...
[ETL Pipeline (etl/)] ──(ETL 2단계: Gemini API 요약)──► [SQLite3 DB]
       │                                                    ▲
       │ (ETL 3단계: Geocoding REST API 위치 보정)          │ (Deep Research)
       └────────────────────────────────────────────────────┘
                               ▲
                               │ API 요청
                               ▼
...
```bash
# VWorld Map Service Key (프론트엔드 테스트용)
NEXT_PUBLIC_VWORLD_SERVICE_KEY=your_vworld_key_here

# Gemini API Key & Engine Version
GEMINI_API_KEY=your_gemini_key_here
GEMINI_ENGINE_VERSION=gemini-2.0-flash

# YouTube API Key (할당량 초과 시 스크래퍼 백업 동작)
YOUTUBE_API_KEY=your_youtube_key_here

# Geocoding REST API Key (Kakao Local / Naver Maps)
GEOLOCATION_API_KEY=your_geocoding_key_here
```
