...
def build_place_export(summaries: list[PlaceSummary], export_format: str) -> ExportPayload:
    ...
    if normalized == "xlsx":
        return (
            ...
            "kor-travel-concierge-places.xlsx",
        )
    if normalized == "gpx":
        return (_build_gpx(summaries), "application/gpx+xml", "kor-travel-concierge-places.gpx")
    if normalized == "kml":
        return (
            _build_kml(summaries),
            "application/vnd.google-earth.kml+xml",
            "kor-travel-concierge-places.kml",
        )
...
def _build_gpx(summaries: list[PlaceSummary]) -> bytes:
    ...
    xml = (
        ...
        '<gpx version="1.1" creator="kor-travel-concierge" '
...
def _build_kml(summaries: list[PlaceSummary]) -> bytes:
    ...
    xml = (
        ...
        "<Document>"
        "<name>kor-travel-concierge 장소 목록</name>"
        f'{"".join(placemarks)}'
        "</Document>"
        "</kml>"
    )
