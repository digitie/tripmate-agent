...
- **T-019 완료**: keyword/channel/playlist harvest 오케스트레이션을 `pipeline.run_harvest`와 scheduler 기본 `harvest` handler에 연결.
- **다음 착수 대상**: T-011 MCP 서버 읽기/쓰기 UX 구현. REST와 같은 `crawl_runs`, 장소 조회, 보정/병합/검수 도메인 서비스를 재사용해야 한다.
