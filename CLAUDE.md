...
- **T-011 완료**: FastMCP 기반 MCP 서버 읽기/쓰기 도구를 `tripmate_mcp` 패키지에 구현하고, `mcp/server.py`는 Docker Compose 호환 래퍼로 유지.
- **다음 착수 대상**: T-012 Next.js 프론트엔드 스택 정비. Tailwind CSS, shadcn/ui, React Hook Form, Zod, TanStack Query를 실제 화면과 연결해야 한다.
...
- 프론트엔드 검수 UX, Docker Compose 통합 검증, Windows Playwright E2E 검증은 각각 T-012~T-015에 남아 있다.
