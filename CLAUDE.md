...
- **T-009 완료**: `frame_extraction` 대표 프레임 추출, `yt-dlp` 스트림 URL 선택, FFmpeg Input Seeking JPEG 추출, RustFS `frame`/`raw_video` 저장, `video_place_mappings.frame_asset_id` 연결 구현.
- **다음 착수 대상**: T-010 APScheduler 단일 실행자 구현. `crawl_runs.pending` 작업을 claim하고 T-006~T-009 파이프라인을 작업 상태와 heartbeat로 실행해야 한다.
...
- T-010 전까지 REST/MCP가 생성한 `crawl_runs.pending` 작업은 실제 백그라운드 ETL 실행자와 아직 연결되지 않는다.
