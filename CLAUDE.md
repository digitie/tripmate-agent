...
Gemini API 기반의 YouTube 여행 컨텐츠 검색, 정리, VWorld 지도 시각화, MCP 읽기/쓰기 도구 UX를 함께 제공하는 `krtour-ai-agent` 개발 초기 단계이다. 최신 기준 문서는 Google Docs `AI유튜브여행_소형프로젝트_SpatiaLite_명세서`와 후속 RustFS 미디어 저장 요구사항이며, 1~2인 운영 기준 소형 프로젝트로 설계를 경량화한다.
...
- **T-033 완료**: RustFS 로컬 개발 설정은 단일 `krtour-map` 버킷, `features/` prefix, 호스트 `http://127.0.0.1:12101`, Compose 컨테이너 `http://host.docker.internal:12101` 기준으로 맞춘다. `http://rustfs:9000`은 선택형 내장 RustFS profile에서만 사용한다.
...
- **T-068 완료**: TripMate feature 연계 POI/curated plan 소비 흐름을 검증했다. `tripmate-agent`는 TripMate DB에 직접 쓰거나 자동 POI/curated plan 등록을 하지 않고, `python-krtour-map`이 `tripmate-agent-youtube` provider로 생성한 feature의 `feature_id`와 `feature_snapshot`을 TripMate가 자체 feature 연계 POI row로 저장하는 흐름을 유지한다. Curated plan은 feature 모음이 아니라 그 POI row들의 모음이다. `docs/feature-export-api.md`를 공급자 정본 계약으로 추가하고, feature export API 테스트에 이름·좌표·8자리 카테고리 제안·YouTube video/channel/playlist 근거·Gemini URL evidence가 snapshot 응답에 보존되는 회귀 검증을 추가했다.
...
- **실행 모델 결정**: 앱 런타임은 단일 호스트 Docker Compose(`docker compose up -d --build`)이며 host 고정 포트 API `12401`, MCP `12402`, Web `12405`로 띄운다(컨테이너 내부 API `8000`, Web `3000`). RustFS는 외부 고정 Docker 서비스로 두고 S3 API `12101`, 콘솔 `12105`를 사용한다. `scripts/start-live.sh`는 기동 전 `scripts/stop-fixed-ports.sh`로 이 repo 소유 고정 포트 `12401`/`12402`/`12405`를 점유한 리스너(Linux/Docker/WSL/Windows)를 회수해 재시작을 보장하고, RustFS 포트는 회수하지 않는다(패턴은 `python-krtour-map`에서 차용). Windows 사용자는 WSL2(Ubuntu) + Docker 안에서 동일한 bash 명령으로 앱을 구동한다(ADR-23). **E2E Playwright만 Windows 호스트에서 실행**한다(ADR-23 예외). 에이전트/Codex 작업 명령은 `git` 명령과 Windows Playwright E2E를 제외하고 WSL2(Ubuntu) bash에서만 실행한다.
...
```
F:\dev\krtour-ai-agent\
├── frontend/             # Next.js App Router 프론트엔드 (Compose host Port: 12405 → 컨테이너 3000)
│   ├── src/
...
│   ├── package.json
│   └── tsconfig.json
├── backend/              # FastAPI 비동기 API 백엔드 (Compose host Port: 12401 → 컨테이너 8000)
│   ├── app/
...
│   └── server.py         # krtour_ai_mcp.server 호출
├── krtour_ai_mcp/        # MCP 서버 읽기/쓰기 도구 UX 구현
...
DATABASE_URL=postgresql+asyncpg://addr:addr@localhost:5432/krtour_ai_agent python main.py  # 12401 포트 구동
```
