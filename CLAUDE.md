...
- **T-068 완료**: TripMate curated plan/POI 소비 흐름을 검증했다. `tripmate-agent`는 TripMate DB에 직접 쓰거나 자동 curated plan 등록을 하지 않고, `python-krtour-map`이 `tripmate-agent-youtube` provider로 생성한 feature의 `feature_id`와 `feature_snapshot`을 TripMate가 수동 선택·저장하는 흐름을 유지한다. `docs/feature-export-api.md`를 공급자 정본 계약으로 추가하고, feature export API 테스트에 이름·좌표·8자리 카테고리 제안·YouTube video/channel/playlist 근거·Gemini URL evidence가 snapshot 응답에 보존되는 회귀 검증을 추가했다.
...
- **다음 착수 대상**: T-069 통합 검증과 운영 문서 정리. (T-067 consumer/import는 `python-krtour-map` origin/main에 이미 머지됨 — PR #346/#347/#345. T-068 소비 흐름은 자동 등록 없음·수동 선택 유지로 확정. T-070 `category_code_suggestion`도 consumer가 수신.)
