...
- **T-010 완료**: APScheduler 단일 실행자 `scheduler.worker`가 stale 재투입, pending claim, heartbeat/progress, done/failed 상태 전이를 처리하고 keyword harvest를 `pipeline.run_harvest`에 연결.
- **다음 착수 대상**: T-019 channel/playlist harvest 오케스트레이션 보강. API는 `channel_id`/`playlist_id`를 받지만 현재 기본 scheduler handler는 keyword harvest만 실행한다.
...
- channel/playlist harvest는 `YouTubeClient` 저수준 메서드는 있으나 end-to-end 오케스트레이션이 아직 없다(T-019).
- MCP 쓰기 도구, 프론트엔드 검수 UX, Docker Compose 통합 검증, Windows Playwright E2E 검증은 각각 T-011~T-015에 남아 있다.
