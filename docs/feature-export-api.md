...
- `krtour-ai-agent`는 `feature_id`를 만들지 않는다. `feature_id`와 최종
  `feature_snapshot` 생성은 `python-krtour-map` 책임이다.
- TripMate는 `krtour-ai-agent` DB에 직접 붙지 않는다. TripMate의 여행 POI와 curated
...
- `source_record`: provider `krtour-ai-agent-youtube`, dataset
...
`status='inactive'` 전환으로 처리한다. `krtour-ai-agent`는 RustFS 객체나 과거 원본을
삭제하지 않는다.
