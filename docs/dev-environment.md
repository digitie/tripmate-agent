...
대표 프레임 추출 runtime은 `FFMPEG_PATH` 환경변수에 지정된 실행 파일만 직접 사용합니다. Windows live에서는 `.local\ffmpeg` 아래의 프로젝트 로컬 바이너리를 사용하고, 해당 바이너리는 gyan.dev `.sha256` sidecar 검증을 통과한 아카이브에서만 준비합니다. `FFPROBE_PATH`는 Windows live 사전 검증에서 `ffprobe -version`을 확인하기 위한 스크립트 관리 값이며, API·scheduler·frontend runtime에는 주입하지 않습니다. Docker Compose에서는 Windows 경로가 컨테이너에 새지 않도록 `DOCKER_FFMPEG_PATH`를 컨테이너 내부 `FFMPEG_PATH`(`/usr/bin/ffmpeg`)로만 주입합니다.
