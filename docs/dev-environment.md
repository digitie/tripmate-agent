...
본 문서는 `krtour-ai-agent` 프로젝트의 프론트엔드, 백엔드, ETL 파이프라인을 **Linux Docker**(앱 런타임/배포 전용, ADR-23)에서 빌드·실행하고, **E2E Playwright 테스트를 Windows 호스트에서 실행**(ADR-23 예외)하기 위한 상세 절차를 다룹니다. Windows 사용자는 앱 구동에 한해 WSL2(Ubuntu) + Docker를 사용합니다.
...
- **PostgreSQL + PostGIS**: 로컬 개발은 `python-kraddr-geo`가 쓰는 PostgreSQL/PostGIS 서버를 재사용하되 별도 DB `krtour_ai_agent`를 사용합니다.
...
4. `.env`에 PostgreSQL/PostGIS DSN을 지정합니다:
   ```dotenv
   DATABASE_URL=postgresql+asyncpg://addr:addr@localhost:5432/krtour_ai_agent
   ```
...
3. `rustfs`, `api`, `mcp`, `scheduler`, `frontend`를 단일 프로젝트(`krtour-ai-agent-verify`)로 실행합니다.
...
- **해결책**: `python-kraddr-geo` 로컬 DB 서버가 실행 중인지, `krtour_ai_agent` DB가 생성되어 있는지, `DATABASE_URL`의 driver·host·port·user·password·DB 이름이 ADR-25와 맞는지 확인합니다. PostGIS 함수 오류가 나면 `CREATE EXTENSION postgis` 적용 여부를 확인합니다.
