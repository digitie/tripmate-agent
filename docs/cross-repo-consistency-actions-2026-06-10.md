# cross-repo 일관성 검토 — kor-travel-concierge 측 반영 항목 (2026-06-10)

> **출처**: python-krtour-map repo에서 수행한 3-시스템(krtour-map · TripMate ·
> kor-travel-concierge) 완성도·정합성 교차 검토의 kor-travel-concierge 측 산출물.
...
> **기준 커밋**: kor-travel-concierge `origin/main` `a443ca0`(#55) · krtour-map `origin/main`
...
|---|---|---|
| 호출 경로 | `GET {base}/api/v1/features/{snapshot\|changes}` — **중립 경로 정렬 완료** (krtour T-217a, krtour-map#346 머지 2026-06-11). endpoint 선택은 consumer 설정 `kor_travel_concierge_feature_sync_endpoint` | `packages/krtour-map-dagster/.../provider_fetchers.py` |
| 인증 | `X-API-Key` 헤더 (kor-travel-concierge `API_KEYS` 중 하나) | 같은 파일 |
| 요청 파라미터 | `limit`(krtour 설정 `kor_travel_concierge_feature_page_size`, **상한 500** — 본 repo `FEATURE_EXPORT_LIMIT_MAX`와 정렬됨), `cursor`(opaque) | 〃 |
...
| krtour 측 env | `KRTOUR_MAP_KTC_BASE_URL`, `KRTOUR_MAP_KTC_API_KEY`, timeout | `:72-84` |
| provider 식별 | provider `kor-travel-concierge-youtube`, dataset `youtube_place_candidates` | krtour `src/krtour/map/providers/kor_travel_concierge.py` |
| operation 처리 | `upsert` 적재 + **`reject`/`tombstone` → 대응 feature `status='inactive'` 전환**(krtour T-217b 구현 완료, ADR-050 #4 — MOIS Step C 동형. 부가 `rejection_reason`은 krtour 비적재라 무시) | krtour `providers/kor_travel_concierge.py` + Dagster asset |
...
- **TA-01 🔴 — T-066 export API 구현 시 준수 체크리스트**
  ...
  - [ ] `limit` 쿼리 파라미터 수용 (기본/최대값은 본 repo가 정의하되 krtour 설정
        `kor_travel_concierge_feature_page_size`와 운영 합의)
...
- **TA-04 ✅ — RustFS 버킷 소유권** (의사결정 D-01 후속 반영)
  - 기존 기본값은 consumer repo와 공유되는 버킷명(prefix `features/`)이었고,
    krtour의 백업/수명주기 책임과 충돌 소지가 있었다.
  - **2026-06-13 후속 반영**: `RUSTFS_BUCKET_RAW_VIDEOS/SUBTITLES/FRAMES`와
    공개 객체 URL 기준을 전용 버킷 `kor-travel-concierge`로 분리했다. `python-krtour-map`은
    consumer repo 이름으로만 남고 RustFS 버킷 소유권은 공유하지 않는다.
...
이번 교차 검토에서 kor-travel-concierge 문서는 세 repo 중 **일관성 최상** 평가
